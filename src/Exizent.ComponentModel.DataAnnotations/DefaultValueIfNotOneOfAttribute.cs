...
public class DefaultValueIfNotOneOfAttribute : DependantPropertyBaseAttribute
{
    public object[] DependantPropertyRequiredValues { get; }

    public DefaultValueIfNotOneOfAttribute(string dependentProperty, params object[] dependantPropertyRequiredValues)
        : base(dependentProperty, "The field {0} must not be set when {1} is one of {2}.")
    {
    ...
    protected override bool IsValid(object? value, object? dependentPropertyValue)
    {
        ...
        if (!DependantPropertyRequiredValues.Any(x => Equals(dependentPropertyValue, x)))
        {
            ...
            if (AllowEmptyStrings && (value is string stringValue) && stringValue.Trim().Length == 0)
                return true;
