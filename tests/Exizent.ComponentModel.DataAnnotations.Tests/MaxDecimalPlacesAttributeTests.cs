...
public class MaxDecimalPlacesAttributeTests
{
    ...
    [InlineData(1.00000)]
    [InlineData(1.0001)]
    public void ShouldBeValidWhenMaxDecimalPlacesIsHonoured(decimal value)
    {
    ...
    [InlineData(0.00001)]
    [InlineData(99.99991)]
    public void ShouldBeInvalidWhenMaxDecimalPlacesExceeded(decimal value)
    {
        var model = new TestModel { Value = value };
        var context = new ValidationContext(model);
        var results = new List<ValidationResult>();
        ...
        results[0].ErrorMessage.Should()
            .Be($"The field {nameof(TestModel.Value)} must have a max of 4 decimal places.");
    ...
    {
        var act = () => new MaxDecimalPlacesAttribute(maxDecimalPlaces);
